<script lang="ts">
  ...
  // Svelte props are live bindings — the rAF loop reads the latest
  // values directly, so the main onMount never needs to re-run.
  let {
  ...
  // The main mount parks the rAF loop when steam is disabled or eco
  // mode is on — keeping the loop ticking just to clear an empty canvas
  // each frame is wasted main-thread time. The effects below resume the
  // loop the moment the props transition back into "active" state.
  ...
  onMount(() => {
    ...
    function renderFrame(): void {
      ...
      gl.uniform1f(maxIntensityUniformLocation, Math.max(1e-4, maxIntensity));
      gl.uniform1f(opacityUniformLocation, Math.max(0, Math.min(1, opacity)));
      gl.uniform1f(whitelightScaleUniformLocation, whitelightScale);
      gl.uniform1i(showAtlasUniformLocation, showAtlas ? 1 : 0);
      gl.drawArrays(gl.TRIANGLES, 0, 6);
      gl.bindVertexArray(null);
    }
  ...
  // active state. The main mount reads `enabled`/`ecoMode` live, so it
  // never tears down on these changes; this effect just pokes the
  // existing loop awake.
  $effect(() => {
    if (enabled && !ecoMode) {
      wakeup?.();
  ...
  // the gating conditions itself, so this listener just calls it.
  onMount(() => {
...
<canvas bind:this={canvasElement} class="steam-compositor-canvas"></canvas>
